...
class OVRPBLTWEnv:
    ...
    def step(self, selected, out_reward = False, soft_constrained=False, backhaul_mask = "hard"):
        ...
        # meet the constraints of backhaul, never do negative deliveries [hard]
        ...
        self.constraints_ninf_flag[out_of_tw] += 1.
        ...
        ####################################
        # out of time:
        # timeout value of the selected node = (current time - service time) - tw_end
        ...
        total_timeout = torch.clamp(total_timeout - round_error_epsilon, min=0)# negative value means current time < tw_end, turn it into 0
        self.timeout_list = torch.cat((self.timeout_list, total_timeout[:, :, None]), dim=2) # shape: (batch, pomo)
        if not soft_constrained:
            if (total_timeout!=0).any():
                print("out of tw")
        # out of duration limit:
        # out_of_dl value = current_length - route_limit
        ...
        if not soft_constrained:
            if (out_of_dl!=0).any():
                print("out of dl")
        # out of capacity:
        # out_of_capacity value 1 = -remaining capacity (i.e., remain negative capacity means already exceeding, positive means real remaining)
        ...
        if done:
            if not out_reward:
                ...
                reward = [dist_reward, total_timeout_reward, timeout_nodes_reward, total_out_of_dl_reward,
