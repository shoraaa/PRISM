...
class Reset_State:
    ...
    # shape: (batch, problem)
    vehicle_number: torch.Tensor = None
    # shape: (batch, 1)
...
class Step_State:
    ...
    # shape: (batch, pomo)
    vehicle_remaining: torch.Tensor = None
    # current_capacity_remaining: torch.Tensor =None
    # current_capacity_remaining = vehicle_remaining * capacity + load
    # shape: (batch, pomo)
...
class VRPTWEnv:
    def __init__(self, **env_params):
        ...
        self.limited_vehicle_number = env_params['limited_vehicle_number']
    ...
    def load_problems(self, batch_size, problems=None, aug_factor=1,normalize=True):
        ...
        if self.limited_vehicle_number is not None:
            depot_xy, node_xy, node_demand, service_time, tw_start, tw_end, vehicle_number = data
        else:
            depot_xy, node_xy, node_demand, service_time, tw_start, tw_end = data
        ...
        if aug_factor > 1:
            if aug_factor == 8:
                ...
                if self.limited_vehicle_number is not None:
                    vehicle_number = vehicle_number.repeat(8, 1)
            else:
        ...
        if self.limited_vehicle_number is not None:
            self.reset_state.vehicle_number = vehicle_number
    ...
    def reset(self):
        ...
        # shape: (batch, pomo)
        if self.limited_vehicle_number is not None:
            self.vehicle_remaining = self.vehicle_number.repeat(1, self.pomo_size)
    ...
    def pre_step(self):
        ...
        if self.limited_vehicle_number is not None:
            self.step_state.vehicle_remaining = self.vehicle_remaining
    ...
    def step(self, selected, visit_mask_only =True, out_reward = False, simulation=False, infsb_level=False, safety_layer=None, use_sl_mask=False, simulate_and_val=False):
        ...
        self.vehicle_remaining[self.at_the_depot] -= 1 # start a new vehicle at the depot
        ...
        self.tw_ninf_flag[out_of_tw] = float('-inf')
        self.tw_ninf_flag[fail_return_depot] = float('-inf')

        self.ninf_mask_copy = self.ninf_mask.clone()
        if simulation and self.selected_count < self.problem_size - 1 and self.selected_count >= 2:
            unvisited = self.visited_ninf_flag != float('-inf')
            unvisited[:, :, 0] = False
            # no need to consider opening new vehicle FIXME
            first_demand = self.depot_node_demand.unsqueeze(1).repeat(1, self.pomo_size, 1)
            first_demand_unvisited = torch.where(unvisited == False, 0., first_demand)
            first_remaining = self.load.unsqueeze(-1) - first_demand_unvisited
            first_need_new_vehicle = first_remaining + round_error_epsilon < 0.
            first_need_new_vehicle[:, :, 0] = True
            remaining_demand_total = first_demand_unvisited.sum(-1).unsqueeze(-1) - first_demand_unvisited
            updated_load = self.load.unsqueeze(-1).expand(-1, -1, self.problem_size + 1)
            updated_load = torch.where(first_need_new_vehicle, 0., updated_load)  # open new vehicle means clearing the remaining load
            remaining_capacity_total = updated_load + self.vehicle_remaining.unsqueeze(-1)
            remaining_capacity_total = remaining_capacity_total - first_demand_unvisited  # minus the corresponding node
            simulated_ninf_flag = remaining_capacity_total < remaining_demand_total + round_error_epsilon  # capacity < demand meanas infeasible
            self.simulated_ninf_flag = torch.zeros((self.batch_size, self.pomo_size, self.problem_size + 1))
            self.simulated_ninf_flag = torch.where(simulated_ninf_flag, float('-inf'), self.simulated_ninf_flag)

            if not use_sl_mask: # use simulated mask once not using sl_mask
                self.ninf_mask = torch.where(self.simulated_ninf_flag==float('-inf'), float('-inf'), self.ninf_mask)
                all_infsb = ((self.ninf_mask == float('-inf')).all(dim=-1)).unsqueeze(-1).expand(-1, -1, self.problem_size+1)
                # all_infsb = ((self.simulated_ninf_flag==float('-inf')).all(dim=-1)).unsqueeze(-1).expand(-1,-1,self.problem_size)
                self.ninf_mask = torch.where(all_infsb, self.ninf_mask_copy, self.ninf_mask)
        ...
        if done:
            ...
            dist_reward = reward.clone()
            # calculate penalty
            diff = self.selected_node_list.roll(-1) - self.selected_node_list
            used_vehicle_number = self.selected_count - (diff==0).sum(-1) - self.problem_size
            exceed_vehicle_penalty = (used_vehicle_number - self.vehicle_number).int()
            idx = torch.nonzero(self.load_list == 1).reshape(self.batch_size, self.pomo_size, -1, 3)[:, :, 1:, 2] -1
            load_spent = 1. - self.load_list.gather(2, idx)
            load_spent = torch.where(load_spent < round_error_epsilon, 2, load_spent)
            load_spent, _ = torch.sort(load_spent)
            exceed_demand_penalty = torch.zeros_like(exceed_vehicle_penalty).float()
            for b in range(self.batch_size):
                for p in range(self.pomo_size):
                    if exceed_vehicle_penalty[b,p] <= 0: # no penalty for those meet the requirements
                        continue
                    exceed_demand_penalty[b,p] = load_spent[b,p,:exceed_vehicle_penalty[b,p]].sum()
            reward = [reward, -exceed_demand_penalty, -exceed_vehicle_penalty] # note the minus sign!
            infeasible = exceed_vehicle_penalty > 0
        else:
            reward = None
            dist_reward = None

        if not out_reward:
            return self.step_state, dist_reward, done, infeasible
    ...
    def generate_dataset(self, num_samples, problem_size, path):
        ...
        print("Save VRPTW-LV dataset to {}".format(path))
    ...
    def load_dataset(self, path, offset=0, num_samples=1000, disable_print=True):
        ...
        depot_xy, node_xy, node_demand, capacity, service_time, tw_start, tw_end, vehicle_number = [i[0] for i in data], [i[1] for i in data], [i[2] for i in data], [i[3] for i in data], [i[4] for i in data], [i[5] for i in data], [i[6] for i in data], [i[7] for i in data]
        depot_xy, node_xy, node_demand, capacity, service_time, tw_start, tw_end, vehicle_number = torch.Tensor(depot_xy), torch.Tensor(node_xy), torch.Tensor(node_demand), torch.Tensor(capacity), torch.Tensor(service_time), torch.Tensor(tw_start), torch.Tensor(tw_end), torch.Tensor(vehicle_number)
        node_demand = node_demand / capacity.view(-1, 1)
        data = (depot_xy, node_xy, node_demand, service_time, tw_start, tw_end, vehicle_number.unsqueeze(-1).int())
        return data
    ...
    def get_random_problems(self, batch_size, problem_size, normalized=True, ):
        ...
        if instance_invalid:
            ...
            if self.limited_vehicle_number == "adaptive":
                vehicle_number = torch.ceil(node_demand.sum(-1)).int().unsqueeze(-1)
                return depot_xy, node_xy, node_demand, service_time, tw_start, tw_end, vehicle_number
            if isinstance(self.limited_vehicle_number, int):
                vehicle_number = torch.ceil(node_demand.sum(-1)).int().unsqueeze(-1) + self.limited_vehicle_number
                return depot_xy, node_xy, node_demand, service_time, tw_start, tw_end, vehicle_number
            ...
            if self.limited_vehicle_number == "adaptive":
                vehicle_number = torch.ceil(node_demand.sum(-1) /float(demand_scaler)).int()
                return depot_xy, node_xy, node_demand, capacity, service_time, tw_start, tw_end, vehicle_number
            if isinstance(self.limited_vehicle_number, int):
                vehicle_number = (torch.ceil(node_demand.sum(-1) /float(demand_scaler)).int() + self.limited_vehicle_number)
                return depot_xy, node_xy, node_demand, capacity, service_time, tw_start, tw_end, vehicle_number
