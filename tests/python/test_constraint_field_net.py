...
def test_constraint_field_net_uses_normalized_decoder_contract() -> None:
    ...
    # The per-edge half is zero at init, so a fresh model is exactly the
    # graph-level-coupler policy.
    edge_index = torch.zeros(5, dtype=torch.long)
    assert torch.equal(
        model.state_energy(output, live_state, edge_index),
        torch.zeros(5, channel_count),
    )
    # An off-graph transition contributes no state-conditioned energy.
    off_graph = torch.full((5,), -1, dtype=torch.long)
    assert torch.equal(
        model.state_energy(output, live_state, off_graph),
        torch.zeros(5, channel_count),
    )
...
def test_typed_field_accepts_unseen_runtime_resource_without_new_weights() -> None:
    ...
    # The per-edge half is aligned to the candidate graph, like the field.
    assert output["state_field"].shape == (
    ...
    traced = battery.sample_traced(
        ...
        edge_state_field=output["state_field"].numpy(),
...
def test_cpp_trace_replays_exact_state_dependent_policy() -> None:
    ...
    traced = decoder.sample_traced(
        ...
        edge_state_field=output["state_field"].detach().numpy(),
