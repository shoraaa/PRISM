...
# VRPTW is the compositional bridge: it activates only the time-window
# resource while allowing depot-separated routes. CVRPTW and OCVRPTW then
# teach interactions with capacity/open routing in later curriculum phases.
...
def _generated_vrptw(size: int) -> dict:
    ...
    # Every customer can therefore be served on its own route, while sampled
    # windows still determine which customers can profitably share a route.
    horizon = 3.2
    earliest = travel
...
def _load_pickle_data(path: Path, name: str, start: int, count: int) -> dict:
    ...
    if "tw" in name:
        result["service_time"] = torch.cat(
            (torch.zeros(count, 1), torch.tensor([row[-3] for row in rows], dtype=torch.float32)), 1
        )
        result["tw_start"] = torch.cat(
            (torch.zeros(count, 1), torch.tensor([row[-2] for row in rows], dtype=torch.float32)), 1
        )
        result["tw_end"] = torch.cat(
            (torch.full((count, 1), 3.0), torch.tensor([row[-1] for row in rows], dtype=torch.float32)), 1
        )
    return result
...
def _load_tensor_data(path: Path, name: str, start: int, count: int) -> tuple[dict, float | None]:
    ...
    for field in ("service_time", "tw_start", "tw_end"):
        ...
        if values.shape[1] != node_count:
            depot_value = 1.0 if field == "tw_end" and name.startswith("a") else 3.0 if field == "tw_end" else 0.0
...
def load_saved_data(
    ...
    *,
    ...
    reference = _reference(
        None if solution_path is None else Path(solution_path), start, count
    )
