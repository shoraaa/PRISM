...
def validation_metrics_for_wandb(
    ...
    """Drop static validation-manifest bookkeeping from W&B histories."""
    selected = {}
...
class Logger:
    ...
    def log_baseline(self, gap: float) -> None:
        """Store the classical reference as run metadata, not an epoch."""
        if self.use_wandb:
            try:
                import wandb
            except ImportError:
    ...
    def log_validation(
        ...
        values = {
            # Canonical costs are not comparable across distance, penalty, and
            # maximize-prize tasks. Keep the mixed value explicitly diagnostic
            # rather than presenting it as a model-quality aggregate.
        ...
        values.update(
            {
                ...
                for key, value in validation_metrics_for_wandb(
                    metrics
                ).items()
            }
        )
